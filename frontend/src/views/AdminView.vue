...
async function changeRole(user: AdminUser, event: Event) {
  ...
  try {
    ...
    actionError.value = cause instanceof ApiError || cause instanceof Error ? cause.message : '角色更新失败。';
    await loadUsers();
  } finally {
    updating.value = null;
  }
...
<template>
  <section class="admin-stats" aria-label="用户统计">
    <UiCard as="article" class="stat-card"><span class="stat-icon"><Users :size="18" aria-hidden="true" /></span><strong>{{ users.length }}</strong><span>全部账号</span></UiCard>
    <UiCard as="article" class="stat-card"><span class="stat-icon"><ShieldCheck :size="18" aria-hidden="true" /></span><strong>{{ adminCount }}</strong><span>管理员</span></UiCard>
  </section>
  <UiCard as="section" class="panel table-panel">
    <div v-if="loading" class="admin-skeleton" role="status" aria-label="正在加载用户列表" aria-busy="true"><span v-for="index in 5" :key="index" /></div>
    ...
    <div v-else class="table-wrap">
      ...
      <p v-if="actionMessage" class="admin-feedback form-success" role="status" aria-live="polite">{{ actionMessage }}</p>
