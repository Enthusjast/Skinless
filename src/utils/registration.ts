...
export function parseRegistrationSettings(
  ...
  if (!input)
    return invalid("invalid_settings", "A JSON settings object is required.");
  ...
  if (
    ...
    return invalid(
      ...
      "registrationMode must be open, invite or closed.",
    );
  }

  const maxProfilesPerUser =
    input.maxProfilesPerUser === undefined
      ? current.maxProfilesPerUser
      : input.maxProfilesPerUser;
  if (!boundedInteger(maxProfilesPerUser, 1, 50)) {
    return invalid(
      "invalid_profile_quota",
      "maxProfilesPerUser must be an integer from 1 to 50.",
    );
  }

  const maxTexturesPerUser =
    input.maxTexturesPerUser === undefined
      ? current.maxTexturesPerUser
      : input.maxTexturesPerUser;
  if (!boundedInteger(maxTexturesPerUser, 1, 500)) {
    return invalid(
      "invalid_texture_quota",
      "maxTexturesPerUser must be an integer from 1 to 500.",
    );
  }

  const enforceJoinIp =
    input.enforceJoinIp === undefined
      ? current.enforceJoinIp
      : input.enforceJoinIp;
...
export function parseFutureTimestamp(
  value: unknown,
  now: number,
...
export function parseInviteInput(
  ...
  if (!input)
    return invalid("invalid_invite", "A JSON invite object is required.");

  const rawUseLimit = input.useLimit ?? input.uses ?? 1;
  if (!boundedInteger(rawUseLimit, 1, 1_000)) {
    return invalid(
      "invalid_invite_use_limit",
      "useLimit must be an integer from 1 to 1000.",
    );
  }
  ...
  if (typeof note !== "string")
    return invalid("invalid_invite_note", "note must be a string.");
